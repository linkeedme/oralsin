  ...
  return (
    <section id="investimento" className="bg-white py-24">
      <div className="container-tight">
        ...
        <Reveal delay={150}>
          <div className="mt-8 grid gap-5 rounded-xl2 border border-[var(--line)] bg-[var(--bg)] p-7 sm:grid-cols-2">
            <div>
            ...
            <div>
              <p className="text-sm font-semibold text-[var(--ink-strong)]">
                Custos externos (fora da mensalidade)
              </p>
              <ul className="mt-2 space-y-1.5">
                {investment.externalCosts.map((c) => (
                  <li key={c.name} className="text-xs text-[var(--ink-soft)]">
                    <span className="font-medium text-[var(--ink)]">{c.name}</span> — {c.detail} ·{" "}
